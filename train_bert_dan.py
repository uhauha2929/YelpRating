...
from config import conf_bert_dan, DEVICE
from dataset_bert import ProductUserDatasetBERT
from models.hierarchical_bert_dan import HierarchicalJointModelBERTDAN

log_dir = 'log_bert_dan'
...
def main():
    ...
    train_loader = DataLoader(dataset=train_data, batch_size=conf_bert_dan.batch_size)
    val_loader = DataLoader(dataset=val_data, batch_size=conf_bert_dan.batch_size)

    model = HierarchicalJointModelBERTDAN(conf_bert_dan.bert_dim,
                                          conf_bert_dan.hidden_size).to(DEVICE)

    optimizer = torch.optim.Adam(model.parameters(), lr=conf_bert_dan.learning_rate)

    best_acc = -np.inf

    for i in range(1, conf_bert_dan.epoch + 1):
        train_losses_dict = train(train_loader, model, optimizer)
        metric = evaluate(model, val_loader)

        np.save('{}/bert-dan-epoch-{:02}-loss'.format(log_dir, i), train_losses_dict)
        print(metric)

        if metric['accuracy'] > best_acc:
            best_acc = metric['accuracy']
            torch.save(model.state_dict(), '{}/bert_dan.pt'.format(log_dir))
