...
from config import conf_elmo, DEVICE
...
def main():
    ...
    model = HierarchicalJointModelELMo(conf_elmo.options_file, conf_elmo.weight_file,
    ...
    for i in range(1, conf_elmo.epoch + 1):
        ...
        np.save('{}/elmo-epoch-{:02}-loss'.format(log_dir, i), train_losses_dict)
        print(metric)

        if metric['accuracy'] > best_acc:
            best_acc = metric['accuracy']
            torch.save(model.state_dict(), '{}/elmo.pt'.format(log_dir))
